...
const FROM_EMAIL = process.env.EMAIL_FROM || "TaxEase Nigeria <onboarding@resend.dev>";
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: "Reset your TaxEase password",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
        <div style="text-align: center; margin-bottom: 32px;">
          <h1 style="font-size: 24px; color: #1a3639; margin-bottom: 8px;">TaxEase Nigeria</h1>
        ...
        <p style="font-size: 12px; color: #9ca3af; text-align: center;">
          TaxEase Nigeria — Simplify your tax returns
        </p>
      </div>
    `,
  });
